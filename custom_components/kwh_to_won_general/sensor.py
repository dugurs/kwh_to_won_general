    ...
    for sensor_type in SENSOR_TYPES:
        if sensor_type == "kwhto_won_prev":
            if (prev_usekwh_entity == None or prev_usekwh_entity == ''):
                ...
                leading_entity = prev_leading_entity
                usekwh_entity = prev_usekwh_entity
...
class ExtendSensor(SensorBase):
    ...
    def __init__(self, hass, device,
        ...
        cfg = {
            ...
            'contractKWh': contractKWh_config, # 계약전력
            'welfareDcCfg' : welfare_dc_config, # 복지 요금할인
        }
        self.KWH2WON = K2WAPI(cfg)
        self._usekwh = self.setStateListener(hass, self._usekwh_entity, self.usekwh_state_listener)
    ...
    @callback
    def usekwh_state_listener(self, event: Event[EventStateChangedData]) -> None:
    ...
    def update(self):
        """Update the state."""
        if (self._usekwh != None) :
            if self._calibration > 0: # 보정계수가 적용
                self._usekwh = round(self._usekwh_row * self._calibration , 1)

            if self._sensor_type == "kwhto_kwh": # 보정된 에너지 값 센서
                self._state = self._usekwh
                self._extra_state_attributes['측정사용량'] = self._usekwh_row
                ...
                forcest = self.KWH2WON.energy_forecast(self._usekwh, datetime.datetime.now())
                ...
                if self._sensor_type == "kwhto_won": # 전기 사용 요금
                    ret = self.KWH2WON.kwh2won({'usekwh':self._usekwh,'lagging':self._lagging,'leading':self._leading}, datetime.datetime.now())
                elif self._sensor_type == "kwhto_won_prev": # 전기 전월 사용 요금
                    ret = self.KWH2WON.kwh2won({'usekwh':self._usekwh,'lagging':self._lagging,'leading':self._leading}, self.KWH2WON.prev_checkday(datetime.datetime.now()))
                else: # 예상 전기 사용 요금
                    # self.KWH2WON.calc_lengthDays() # 검침일, 월길이 재계산
                    forcest = self.KWH2WON.energy_forecast(self._usekwh, datetime.datetime.now())
                    ret = self.KWH2WON.kwh2won({'usekwh':forcest['forcest'],'lagging':self._lagging,'leading':self._leading}, datetime.datetime.now())
            ...
            self._prev_usekwh = self._usekwh
